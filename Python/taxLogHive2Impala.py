...
for str in queryDate :
    hiveStatementForPythonCreate += " or date_time like '"+str+"'"
...
for deltime in deleteTime :
    hadoopy.rmr('"/user/hive/warehouse/tax.db/tax_access_log_partition/date_hour='+deleteTime+'"')
...
impalaStatementForCreate += "SELECT client_ip,client,userid,request,method,uri,protocal,path,params,query,fileType,fileName,status,bytes_sent, date_time,referer,useragent,host,concat(strleft(from_unixtime(unix_timestamp(date_time)),14),'00:00')as date_hour from  tax.tax_access_log_python;";
